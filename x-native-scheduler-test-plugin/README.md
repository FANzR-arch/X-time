...
3. 选择顶部“原创排期”或“回复排期”。
4. 搜索并选择浏览器提供的完整 IANA 目标时区，例如 `UTC−07:00 · 旧金山`；需要长期使用时点击“保存为默认时区”。排期窗口和文档时间都按目标时区计算。
...
- 找不到某条回复框的原生排期按钮时，先清空并关闭插件刚填入的回复草稿，再静默跳过并继续；绝不改成立即回复。
...
“首日开始方式”默认是“今天智能开始”：目标时区当前时间仍在窗口内时，从当前时间加 10 分钟开始；当前时间已经超过每日结束时，顺延到次日 `08:00`。切换到“固定每日开始时间”后，今天的固定开始时间未过就从今天开始，已过则从次日开始。手动填写“排期开始”时，以手动时间为准。

开启“不规则排期”后，插件只会给系统自动生成的时间加入 `±N` 分钟随机波动；文档指定和手动指定时间不会被随机改动。

如果后续自动排期超过当天窗口，插件会顺延到次日发布窗口继续排列。
...
- 回复排期第一版仅支持纯文字。
- X 当前页面若不支持某条回复原生排期，该条会安全跳过；登录、页面或目标帖错误仍会中断。
- 暂不支持 GIF、投票、线程和长文。
