...
本地 Chrome/Edge 测试插件，支持原创帖队列和纯文字定时回复队列。原创可保存草稿或使用 X 原生定时；回复任务由扩展持久保存，并在目标时间自动打开帖子发送。
...
8. 查看执行日志；回复失败后使用“恢复未发送任务”，已发送项目不会重复。
...
- X 没有原生回复排期；任务由扩展使用 Chrome alarms 保存和触发。
- 到点后才打开目标帖子和填写回复，最终按钮必须明确显示 `Reply / 回复 / 回覆` 才会点击。
- 找不到目标帖、回复编辑器或明确回复按钮时立即停止。
- 失败状态会保存；恢复时不会再次执行已经标记为已发送的回复。
- 浏览器关闭期间错过的任务会在下次启动后尽快恢复。
- 如果扩展在点击发送期间中断，会标记为“结果未知”，不会自动重发。
...
- 回复排期第一版仅支持纯文字。
- 定时回复要求目标时间 Chrome/Edge 正在运行且 X 账号保持登录。
- 暂不支持 GIF、投票、线程和长文。
- 单个媒体和单次队列总媒体测试限制均为 25MB。
- 运行时需要保持 `x.com` 标签页打开。
- X 前端改版后，按钮或弹窗识别可能失效，需要维护 `content.js`。
- 先用 1-3 条测试，确认到点后回复正确出现在目标帖子下，再批量运行。
