...
5. AI 生成模式：点“复制 AI 提示词”，让 AI 按队列格式输出；把结果粘到左侧输入框后点“解析粘贴”。
6. 模板模式：点“下载模板”，按模板填好后再导入。
7. 如果导入文件里有 `media:` 字段，先在“媒体素材”选择同名图片/小视频。
8. 设置时间来源模式、每日发布窗口、排期策略和操作间隔；不设置开始/结束时，默认按 `08:00-23:00` 自动排。
9. 点击“预览队列”检查时间、内容和媒体预览。
10. 点击“开始”。
11. 查看底部“执行日志”。它会记录目标 X 标签页、页面脚本注入、媒体大小、队列发送和原始报错信息。
12. 完成后打开 X 的 `Unsent posts / Scheduled` 复核。
