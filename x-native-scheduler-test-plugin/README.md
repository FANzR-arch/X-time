...
本地 Chrome/Edge 测试插件，用来手动保存单条帖子，或导入规范 Markdown 批量生成队列，然后选择批量保存为 X 草稿，或通过 X 网页原生定时入口逐条排程。
...
3. 在“批量执行方式”选择“排期发布”或“保存草稿”。选择“保存草稿”时不需要填写任何排期时间。
4. 手动模式：在左侧发帖框里输入一条帖子，可选择或粘贴图片，再点击“保存”加入右侧队列。
5. 导入模式：点击“导入文件”，选择 `.md` 或 `.txt`，插件会自动识别多条帖子并生成右侧队列。
6. AI 生成模式：点“复制 AI 提示词”，让 AI 按队列格式输出；把结果粘到左侧输入框后点“解析粘贴”。
7. 模板模式：点“下载模板”，按模板填好后再导入。
8. 如果导入文件里有 `media:` 字段，先在“媒体素材”选择同名图片/小视频。
9. 排期发布时设置时间来源模式、每日发布窗口、排期策略和操作间隔；不设置开始/结束时，默认按 `08:00-23:00` 自动排。
10. 点击“预览队列”或“预览草稿”检查内容和媒体预览。
11. 点击“开始”或“存草稿”。
12. 查看底部“执行日志”。它会记录目标 X 标签页、页面脚本注入、媒体大小、队列发送和原始报错信息。
13. 完成后打开 X 的 `Unsent posts / Scheduled` 或草稿列表复核。
...
- 如果队列文件声明了 `timezone:`，它必须和当前浏览器时区一致。
- 时区不一致时插件会拒绝导入，避免跨时区定时静默错位。
