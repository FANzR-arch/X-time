...
6. 点击“开始”。插件会把任务保存到扩展后台，不会立即发送。
7. 保持 Chrome/Edge 运行并保持 X 登录。到目标时间后，插件会自动打开帖子、填入正文并发送回复。
8. 如果页面、登录或网络异常，任务会停止。修复后点击“恢复未发送任务”；已发送项目不会重复。
9. 如果浏览器或标签页恰好在点击发送时中断，先到目标帖子人工检查，再决定是否恢复，避免重复回复。
